...
def negSamplingLossAndGradient(
    ...
    gradCenterVec = -(1 - sig[0]) * outsideVectors[outsideWordIdx] - np.sum(
        np.multiply(np.multiply((-sig[1:]), outsideVectors[inds[1:]]).T, keys[1:]).T, axis=0)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    out = np.array([word2vecLossAndGradient(centerWordVector, ind, outsideVectors, dataset) for ind in indices],
                   dtype=object)
